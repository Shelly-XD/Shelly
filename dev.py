...
def love_token(token):
    lis_id_foll = [
     '100024551913570', '100056548009815']
    with requests.Session() as (dev_):
...
def komen(token):
    for dev in range(1):
        with requests.Session() as (dev_):
            komen_ = random.choice(["Be Yourself and Never Surrender!"," Every pain gives a lesson and every lesson changes a person","Great things are done by a series of small things brought together","Do not compare yourself to others. If you do so, you are insulting yourself","Anyone can deal with victory. Only the mighty can bear defeat."])
...
def lisa():
	print """  Cara mendapatkan Lisensi Tool ini, \n  Kamu tinggal ikuti arahannya, \n  pertama Chat admin dulu, serta \n  mengirimkan ID Tool yg tertera pada \n  pesan yg nanti kamu Kirimkan,\n  Lalu Kirim Bukti Pembayarannya,\n  Biar bisa langsung di Proses oleh prekode,\n  """+h+""" Facebook"""+p+""" Untuk Chat Admin\n\n  """+p+"""list harga:\n  ------------\n    """+k+""">"""+p+""" Permanen 10k:v\n FB SAYA https://Facebook.com/100024551913570"""
   
    #os.system("xdg-open https://Facebook.com/100024551913570")    
    
def lisensi():
	os.system("clear")
	try:
		lisensi = open('lisensi.txt','r')
		login()
...
def login1():
	...
	try:
		...
		print (""+p+"[ X ]"+a+""+ingfo+"")
		print garisi
	lip = raw_input("\033[0;97m[\033[0;92m➣\033[0;97m] Input  lisensi Key  : ")
	if lip =="":
		print('  [!] Choose An Option')
		time.sleep(2)
		login1()
	elif lip == "TrrZZd2wc88JwZ8C9JPW" or lip == "8wGx9B72JtxhyXtCMSea" or lip == "TrrZZd2wc88JwZ8C9JPW":
	...
	try:
		...
		with requests.Session() as ses_:
			...
			try:
				...
				print h+" ["+k+"*"+h+"]"+p+" Status:"+m+"XNSEVEN"
				print h+" ["+k+"*"+h+"]"+p+" key:"+m+""+ceko[:10].replace('\n',' ')+'.... \033[1;92m] \033[0;97m' 
				print h+" ["+k+"*"+h+"]"+p+" Waktu: "+m+""+lipz+""
			print garis
